...
if __name__ == '__main__':
	...
	for SNUM in np.arange(1,NUM_SIMS + 1):
		SNUM = 30 * SNUM # Take every 30th freq bin
		# Open the Fits files
		fgd = np.array([fits.getdata("%s/run_%s_s1%03d/%s_%03d.fits"%(dirstr,type_str,SNUM,type_str2,nu*N_SKIP+1),1) for nu in range(N_NU)],dtype=np.float64).T
